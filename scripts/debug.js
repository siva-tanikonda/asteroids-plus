class Debug {
    ...
    static drawDangerLevel(item) {
        var danger = ai.calculateDanger(game.ship, item).toFixed(2).toString();
    ...
    //Draws crosshair on the primary target
    static drawPrimaryTarget(item) {
        ctx.strokeStyle = "rgb(250, 175, 60)";
        ctx.globalAlpha = 0.75;
        ctx.beginPath();
        ctx.arc(item.position.x, item.position.y, 7.5, 0, 2 * Math.PI);
        ctx.moveTo(item.position.x - 10, item.position.y);
        ctx.lineTo(item.position.x + 10, item.position.y);
        ctx.moveTo(item.position.x, item.position.y - 10);
        ctx.lineTo(item.position.x, item.position.y + 10);
        ctx.stroke();
        ctx.globalAlpha = 1.0;
    }
